...
function UploadZone({
  ...
  return (
    <div>
      ...
      <div
        className="relative cursor-pointer overflow-hidden rounded-2xl border-2 border-dashed border-stone-200 bg-stone-50 transition-colors hover:border-yellow-300 hover:bg-yellow-50/40"
        style={{ minHeight: 160 }}
        ...
        <div className="grid grid-cols-2 gap-3">
          {ID_TYPES.map((t) => {
            ...
            return (
              ...
              >
                <span className={selected ? "text-yellow-600" : "text-stone-400"}>{t.icon}</span>
                <span>
      ...
      {step >= 2 && (
        <div className="rounded-2xl bg-white p-6 shadow-[var(--shadow-card)]">
          ...
          {idDocFile && idDocFile.name && (
            <button
              type="button"
              className="mt-4 w-full rounded-full bg-yellow-500 py-2.5 text-sm font-semibold text-white hover:bg-yellow-600"
              onClick={() => setStep(3)}
            >
              Continue →
            </button>
