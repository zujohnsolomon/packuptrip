          ...
          <aside id="price-card" className="space-y-4 lg:sticky lg:top-24 lg:self-start">
            ...
            {isMember && new Date() >= new Date(trip.start_date) && (
              <Link
                ...
                className="flex w-full items-center justify-center gap-2 rounded-2xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm font-semibold text-yellow-700 transition hover:bg-yellow-100"
              >
                🎒 Trip memory
              </Link>
            )}
...
function SuperhostBadge() {
  return (
    <span className="inline-flex shrink-0 items-center gap-0.5 rounded-full bg-yellow-100 px-1.5 py-0.5 text-[10px] font-semibold text-yellow-800">
      ⭐ Superhost
    </span>
  );
}
      ...
      {canMessage && (
        <div className="mt-4 border-t border-stone-100 pt-4">
          <Link
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-stone-200 bg-stone-50 px-4 py-2.5 text-sm font-medium text-stone-700 transition hover:border-yellow-300 hover:bg-yellow-50 hover:text-yellow-700"
          >
