...
import type {
  ...
  PublicContact,
  ReviewWithAuthor,
} from "@/components/host-profile/types";
...
/**
 * Renders the full host profile for an already-resolved profile row.
 * Shared by the canonical `/<username>` route and the `/hosts/<id>` route
 * (the latter only for hosts that haven't set a username yet).
 */
export async function HostProfileView({ profile }: { profile: Profile }) {
