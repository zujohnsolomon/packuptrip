...
export function ImagesEditor({
  ...
  const activeTint =
    accent === "teal" ? "border-green-600 bg-green-50" : "border-indigo-500 bg-indigo-50";
      ...
      >
        ...
        <div className="text-sm font-semibold text-ink">
          ...
          <button
            ...
            disabled={uploading}
            className="ml-1 font-semibold text-indigo-700 underline-offset-2 hover:underline disabled:cursor-not-allowed disabled:opacity-50"
          >
            choose files
          </button>
        </div>
        ...
        {progress && (
          <p className="mt-2 text-xs font-medium text-indigo-700">{progress}</p>
        )}
        {error && (
          ...
          {images.map((url, idx) => (
            ...
            >
              ...
              <input
                ...
                className="block flex-1 rounded-xl border border-stone-200 bg-white px-3.5 py-2 text-sm text-ink placeholder-stone-400 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100"
              />
